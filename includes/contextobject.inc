<?php
  ...
  /* Last modified: 2016-02-23 A. Shields */

/*
    copyright 2016 Good Library, Goshen College
...
class ContextObject
{
  ...
  //return the OCLC number, if any.
  public function getOCN() {
    if (is_array($this->rft_id)) {
      if ($rft_id_length = count($this->rft_id) > 0) {
        for ($i = 0; $i < $rft_id_length; $i++) {
          $rft_id = explode('/', $this->rft_id[$i]);

          if ($rft_id[0] === 'info:oclcnum' && $rft_id[1]) {
            return $rft_id[1];
          }
        }
      }
